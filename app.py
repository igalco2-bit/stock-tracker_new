...
with st.form("add_stock_form", clear_on_submit=True):
    ...
    with col1:
        stock_name = st.text_input("שם המניה בעברית (למשל: שופרסל)")
    with col2:
        stock_ticker = st.text_input("סימול מגלובס (למשל: SAE)")
    ...
    if submit_button:
        if stock_name and stock_ticker:
            ...
            # הוספת סיומת .TA אוטומטית אם היא לא קיימת
            if not clean_ticker.endswith(".TA"):
                clean_ticker += ".TA"
...
if not st.session_state.portfolio.empty:
    ...
    # יצירת טבלה לתצוגה בלי סימול ובלי כמות
